1) Association rule mining w/ multiple minimum supports
	1) Support difference constraint tries to prevent too frequent and too infrequent items from appearing in the same itemset
2) For mining association rules w/ multiple minimum supports
	1) Tail counting is used for rule generation
...
5) For class association rule (CAR) mining
...
6) F3 = {{1, 2, 5}, {1, 2, 3}, {1, 3, 7}, {1, 3, 4}, {3, 4, 7}, {1, 3, 5} {1, 4, 7}}, what are the candidate 4-itemsets after joining and pruning
...
7) For sequential pattern mining
